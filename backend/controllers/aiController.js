...
exports.runHFT = async (req, res) => {
    const { stockSymbol } = req.params;

    try {
        const pythonProcess = spawn("python", ["./ai-engine/ai_hft.py", stockSymbol]);
...
exports.getDynamicStrategy = async (req, res) => {
    const { stockSymbol } = req.params;

    try {
        const pythonProcess = spawn("python", ["./ai-engine/ai_dynamic_strategy.py", stockSymbol]);

        let output = "";
        pythonProcess.stdout.on("data", (data) => output += data.toString());

        pythonProcess.on("close", () => res.json(JSON.parse(output)));
    } catch (error) {
        res.status(500).json({ msg: "Dynamic Strategy Adjustment Failed", error: error.message });
    }
};
